...
export const Signup = () => {
  ...
  const mutation = useMutation(signUpDb, {
    ...
    onError: (error) => {
      ...
      if (errorMessage === "중복된 email 입니다." || errorMessage === "[email] 유효한 이메일 주소를 입력해주세요.") {
        emailRef.current.focus();
      } else if (errorMessage === "[password] 비밀번호는 최소 8자에서 15자 사이로만 가능합니다.") {
