...
export const BoardCommentCard = ({ comment }) => {
    ...
    const requestDeleteComment = async (id) => {
        try {
            await axios.delete(`/boards/${params.id}/comments/${id}`);
            // console.log(response.data);
        } catch (err) {
            // console.log(err);
        }
    ...
    const postLikeClick = async () => {
        try {
            await axios.post(
                `/boards/${params.id}/comments/${comment.commentId}/LIKE`
            );
            // console.log(response.data);
        } catch (err) {
            // console.log(err);
    ...
    const postSadClick = async () => {
        try {
            await axios.post(
                `/boards/${params.id}/comments/${comment.commentId}/SAD`
            );
            // console.log(response.data);
        } catch (err) {
            // console.log(err);
    ...
    const postGratsClick = async () => {
        try {
            await axios.post(
                `/boards/${params.id}/comments/${comment.commentId}/CONGRATULATION`
            );
            // console.log(response.data);
        } catch (err) {
            // console.log(err);
                ...
                <div className="w-full">
                    <div className="flex items-center justify-between mb-3 mr-7">
                        <div className="flex items-center">
                            ...
                            <p className="ml-3 text-xs antialiased text-darkPurple">
                        ...
                        {comment.userName === userName ? (
                            editingComment ? (
                                <div className="flex items-center">
                                    <FaSave
                                        className="text-textPurple cursor-pointer text-lg mr-2"
                                        onClick={() =>
                                            handleEditComment(commentState.text)
                                        }
                                    />
                                    <FaTrashAlt
                                        className="text-textPurple cursor-pointer text-lg"
                                        onClick={() =>
                    ...
                    <div className="flex pl-3 bg-[#ebe0ff] rounded-b-md mr-7 shadow">
                        ...
                        <div className="flex justify-center items-center my-2">
                            ...
                            <span className="text-xs w-4">
                                {comment &&
                                    (comment.congratulationCnt === 0 ||
                                    comment.congratulationCnt === null ? (
                                        ""
                                    ) : (
                                        <span>{comment.congratulationCnt}</span>
                                    ))}
                            </span>
                        </div>
                    </div>
